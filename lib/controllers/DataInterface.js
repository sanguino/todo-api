  ...
  static async mongoConnect() {
    ...
    dbConnectionPromise = new Promise((resolve, reject) => {
      MongoClient.connect(URI, { useNewUrlParser: true }, async (err, db) => {
        ...
        const retrieveTasksById = async data =>
          dbInstance.collection('tasks').findOne({ id: data.id }, FILTER);

        const createTask = async data =>
          dbInstance.collection('tasks').insertOne({ ...data, completed: false });

        const updateTask = async data =>
          dbInstance
            .collection('tasks')
            .updateOne({ id: data.id }, { $set: { completed: data.completed } });
